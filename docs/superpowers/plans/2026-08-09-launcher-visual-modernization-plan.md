...
- [x] Define the three theme presets and a `ThemeColors` value object containing background, surface, elevated surface, accent, border, primary text, and secondary text.
- [x] Define shared `RoundedCornerShape` values for card, dialog, button, field, chip, and navigation surfaces.
- [x] Make Material 3 color schemes consume the resolved palette and animated accent color.
- [x] Persist the selected preset and color overrides with DataStore; fall back to OEM defaults when keys are missing.
...
- [x] Add a three-theme selector with Chinese names and selected-state preview.
- [x] Add color swatches for accent, background, surface, text, and border colors.
- [x] Add a reset-to-theme-default action that clears overrides without clearing unrelated settings.
- [x] Use shared shapes and animated color transitions for the selector.
...
- [x] Replace primary rectangular surfaces with shared card/dialog/button/field shapes.
- [x] Preserve stable grid dimensions and touch targets while rounding only visual containers.
- [x] Use theme border and surface colors instead of hardcoded black/gray borders where the component is user-facing.
- [x] Keep metric readouts monospaced while human-readable Chinese labels use Noto Sans SC.
...
- [x] Scan string literals and `contentDescription` values for user-visible English.
- [x] Translate remaining labels, empty states, errors, dialog actions, and accessibility text.
- [x] Leave widget IDs, API keys, package names, units, media metadata, and protocol values unchanged.
- [x] Run a second scan and manually classify every remaining English literal.
...
- [x] Animate theme color changes over 420ms.
- [x] Smooth compass rotation and preserve shortest-path behavior around 0/360 degrees.
- [x] Keep page transitions short and disable decorative continuous animation.
...
- [x] Run `gradle.bat :app:assembleRelease` with JDK 17 and SDK 36.1.
- [x] Sign with the existing local test keystore so upgrades remain compatible.
- [x] Verify APK with `apksigner verify --verbose` and inspect package metadata with `aapt dump badging`.
- [x] Copy the signed APK to `A:\Desktop`.
